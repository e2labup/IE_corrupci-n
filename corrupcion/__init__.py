   ...
   if player.group.gato == 1:
       ...
       if player.role == C.Ciudadano1_ROLE: #Empieza logica ciudadano1
        ...
        if player.group.soborno_aceptado == 1:
            player.payoff = C.dotacion - cu(3) + 9 - 2*monitores.monto_ciudadano_conSoborno - 2*monitores.ambos_monto_ciudadano_conSoborno
       ...
       if player.role == C.Ciudadano2_ROLE:
        if player.group.soborno_aceptado == 1:
           player.payoff = C.dotacion - cu(18)
           player.Pago_Jugador = player.payoff
        else:
       ...
       if player.role == C.Oficial_ROLE: #Empieza logica Oficial
        ...
        if player.group.soborno_aceptado == 1:
            player.payoff = C.dotacion + cu(3) + 9 - 2*monitores.monto_oficial_conSoborno - 2*monitores.ambos_monto_oficial_conSoborno
            player.Pago_Jugador = player.payoff
        else:
            player.payoff = C.dotacion
            player.Pago_Jugador = player.payoff
...
def calcular_pago_final(self):
    ...
    self.PagoFinalAleatorio = (pago_elegido + 50)*0.1
